...
import type { WorkoutPlanWithDays } from "@strength-sight/shared";

interface Exercise { id?: string; exercise_name: string; sets: number; reps_min: number; reps_max: number; rest_seconds: number; notes: string; order_number?: number; }
interface WorkoutDay { id?: string; day_name: string; day_number: number; exercises: Exercise[]; }
...
export default function EditWorkoutPlan() {
  ...
  async function loadPlan() {
    if (!planId) return;
    try {
      const plan = await apiRequest<WorkoutPlanWithDays>(`/api/workout-plans/${planId}`);
      setPlanName(plan.name);
      setDurationWeeks(String(plan.duration_weeks || 4));
      setDays(plan.workout_plan_days.map((day) => ({
        id: day.id, day_name: day.day_name, day_number: day.day_number,
        exercises: day.workout_plan_exercises.map((ex) => ({ id: ex.id, exercise_name: ex.exercise_name, sets: ex.sets, reps_min: ex.reps_min || 8, reps_max: ex.reps_max || 12, rest_seconds: ex.rest_seconds || 90, notes: ex.notes || "", order_number: ex.order_number })),
      })));
    } catch (error) {
  ...
  function addDay() { const maxDayNumber = days.length > 0 ? Math.max(...days.map(d => d.day_number)) : 0; setDays([...days, { day_name: `Giorno ${maxDayNumber + 1}`, day_number: maxDayNumber + 1, exercises: [] }]); }
  function removeDay(idx: number) {
    if (days.length === 1) { toast({ title: "Errore", description: "Deve esserci almeno un giorno", variant: "destructive" }); return; }
  ...
  async function savePlan() {
    ...
    setSaving(true);
    try {
      await apiRequest(`/api/workout-plans/${planId}`, {
        method: 'PUT',
        body: JSON.stringify({
          name: planName, duration_weeks: parseInt(durationWeeks) || null,
          days: days.map((day, idx) => ({
            id: day.id, day_name: day.day_name, day_number: idx + 1,
            exercises: day.exercises.map((ex, exIdx) => ({ exercise_name: ex.exercise_name, order_number: exIdx + 1, sets: ex.sets, reps_min: ex.reps_min, reps_max: ex.reps_max, rest_seconds: ex.rest_seconds, notes: ex.notes || null })),
          })),
        }),
      });
  ...
  return (
    <div className="min-h-screen px-5 pt-14 pb-24">
          ...
          {days.map((day, idx) => (
            <div key={idx}>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Giorno {idx + 1}</label>
                {days.length > 1 && <button onClick={() => removeDay(idx)} className="text-red-500 hover:bg-red-500/10 p-1 rounded-lg transition-colors"><Trash2 className="w-4 h-4" /></button>}
              </div>
              <input type="text" value={day.day_name} onChange={(e) => updateDayName(idx, e.target.value)} className="w-full h-12 bg-card border border-border rounded-xl px-4 text-foreground outline-none focus:border-primary" />
            </div>
          ))}
          <button onClick={addDay} className="w-full h-12 rounded-xl bg-secondary text-secondary-foreground font-semibold flex items-center justify-center gap-2 transition-colors hover:bg-secondary/80 mt-4"><Plus className="w-5 h-5" />Aggiungi Giorno</button>
        </div>
      )}
      ...
      {step === "exercises" && (
        <div className="space-y-4 mb-8 animate-in fade-in duration-300">
          <div className="flex gap-2 mb-4 overflow-x-auto">
            {days.map((day, idx) => <button key={idx} onClick={() => setCurrentDayIdx(idx)} className={`px-4 py-2 rounded-xl font-semibold whitespace-nowrap transition-colors ${currentDayIdx === idx ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}>{day.day_name}</button>)}
          </div>
      ...
      <div className="fixed bottom-8 left-4 right-4 max-w-[412px] mx-auto flex gap-3">
        <button onClick={() => { if (step === "plan") navigate(-1); else if (step === "days") setStep("plan"); else setStep("days"); }} className="flex-1 h-14 rounded-2xl bg-secondary text-foreground font-semibold transition-transform active:scale-95">Indietro</button>
        {step === "exercises" ? (
          <button onClick={savePlan} disabled={saving} className="flex-1 h-14 rounded-2xl bg-primary text-primary-foreground font-semibold transition-all active:scale-95 disabled:opacity-60">{saving ? "Salvataggio..." : "✓ Salva"}</button>
        ) : (
          <button onClick={step === "plan" ? handlePlanNext : handleDaysNext} className="flex-1 h-14 rounded-2xl bg-primary text-primary-foreground font-semibold flex items-center justify-center gap-2 transition-transform active:scale-95">Avanti<ChevronRight className="w-5 h-5" /></button>
        )}
      </div>
    </div>
  );
